...
impl PendingTransaction {
    ...
    pub fn return_input_signature(&self, input_index: usize, hash_type: SigHashType) -> Result<Hash> {
        let mutable_tx = self.inner.signable_tx.lock()?.clone();
        let verifiable_tx = mutable_tx.as_verifiable();
        let reused_values = SigHashReusedValuesUnsync::new();
        Ok(calc_ecdsa_signature_hash(&verifiable_tx, input_index, hash_type, &reused_values))
    }
